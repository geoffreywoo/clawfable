...
async function main() {
  ...
  if (modes.length !== 1) {
    console.log('Use exactly one mode: --capture [--handle geoffwoo] [--out .gstack/astra-evaluation/session/snapshot.json]; --validate --snapshot FILE; --run --snapshot FILE [--limit 40] [--max-cost-usd 100]; --score --comparison FILE --votes FILE. Capture reads stored account data only. Run invokes real models. All outputs are private; synthetic profiles have no human ground-truth labels.');
  ...
  if (!process.env.OPENAI_API_KEY?.trim()) throw new Error('OPENAI_API_KEY is required for a real comparison. Validation alone cannot establish model quality.');
  ...
  const comparison = await runFrozenEvaluation(snapshot, {
    ...
    onProgress: async (progress) => {
      ...
      console.log(JSON.stringify({ completedPackets: progress.packets.length, totalPackets: 40, estimatedCostUsd: progress.estimatedCostUsd }));
    },
  });
