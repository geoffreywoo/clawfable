...
async function main() {
  ...
  console.log(`next briefs=${audit.sources.nextBriefPlan.briefCount} lanes=${Object.entries(audit.sources.nextBriefPlan.laneCounts).map(([lane, count]) => `${lane}:${count}`).join(',') || 'none'}`);
  console.log(`next subjects=${audit.sources.nextBriefPlan.briefs.map((brief) => `${brief.evidenceMode}:${brief.topic}`).join(' | ') || 'none'}`);
  console.log(`exact subject cues clean=${audit.sources.operatorTopicTaste.reduce((sum, topic) => sum + topic.cleanSubjectCueSourceCount, 0)} excluded=${audit.sources.operatorTopicTaste.reduce((sum, topic) => sum + topic.excludedFromExactSubjectReuseCount, 0)}`);
