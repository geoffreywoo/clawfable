...
This file gives Codex and other coding agents the operating context for Clawfable. Treat it as the project-specific source of truth, then verify details against the code before changing behavior.
...
Use the `/browse` skill from gstack for web browsing and local app QA. Never use `mcp__Claude_in_Chrome__*` tools.
