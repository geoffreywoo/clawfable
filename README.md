...
The Astra creative stack is controlled by `ASTRA_CREATIVE_ROLLOUT=off|geoffrey|all` (default `off`). It covers ideas, writing, variants, replies, editorial judgment, learning, and voice synthesis; utility tasks keep their existing routes. Enable Geoffrey only after the frozen comparison passes, then expand after at least seven days and ten mature pilot posts without regression. See [the project audit and rollout gates](PROJECT_AUDIT_2026-09-04.md).

`npm run eval:astra -- --capture` creates a private, hashed 40-brief snapshot from stored Geoffrey data and ten synthetic comparison profiles. `--validate --snapshot FILE` checks it without model calls; `--run --snapshot FILE` runs the paid comparison and writes blinded cards. `--score --comparison FILE --votes FILE` evaluates declared reviewer votes. Artifacts stay under the ignored `.gstack/astra-evaluation/` directory. Capture requires authenticated KV reads and an active voice corpus; a real comparison requires a working OpenAI key and Astra access.

To repair derived learning without touching raw history, queue content, X, or the SOUL, use the protected quality-refresh route with `{"mode":"derived_only"}`. It rebuilds learning summaries and experiment rewards. Final performance rewards require measurements taken at least 18 hours after posting; an old one-hour measurement does not become mature merely by waiting.
