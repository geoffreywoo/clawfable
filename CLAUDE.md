...
- Throughput contracts (2026-09-26): the uncalibrated Geoffrey AI ambition floor is 0.85 (final copy). Idea-stage AI floors are pre-filters below the final floors (ambition = final - 0.2, frontier lead 0.6); final copy gates stay authoritative. A budgeted live run ideates two briefs (six premises) but funds only the single best idea, whose one writer call returns three variants; a brief whose premise passed but lost the funding slot is not recorded as a failure. The efficient policy gets a 360s run deadline and 220s ideation deadline, and skips the legacy fingerprint pause (it has its own). Failure memory rests a brief 3h after one empty run and 24h only after a repeat, feeds that brief's rejected premises from the last 24h back to ideation, pauses only after five distinct empty runs (2h), and never pauses on `no_qualified_context`. Every finished run logs a copy-free `[gen:run]` line with judge scores; every cron tick logs `[cron:autopilot]`. With Geoffrey auto-post off, the tick still refills the queue to its minimum for review (never posts); other accounts do not draft while auto-post is off.
