  ...
  try {
    ...
    for (const agent of agents) {
      ...
      try {
        ...
        const reason = formatActionError(err, 'cron_agent_tick', {
          handle: `@${agent.handle}`,
        });
        console.error(`[cron] agent ${agent.id} tick failed:`, reason);
        autopilotResults.push({
          agentId: agent.id,
          action: 'error',
          reason,
        ...
        try {
          await addPostLogEntry(agent.id, {
            ...
            xTweetId: '',
            content: '',
            format: 'cron_agent_error',
            topic: 'autopilot',
            postedAt: new Date().toISOString(),
            source: 'cron',
            action: 'error',
            reason,
            errorCode: 'cron_agent_tick',
          });
        } catch (logErr) {
          console.error(`[cron] failed to log agent ${agent.id} tick error:`, logErr instanceof Error ? logErr.message : logErr);
        }
      }
    }
