...
const GEOFFREY_BROAD_SEEDS: FrontierIdeaSeed[] = [
  ...
  },
  {
    ...
    kind: 'ai_product',
    reactionPrompt: 'Use the exact subject for one native move: a blunt valuation or timing call with one subject-specific reason; a real first-person question, desire, or disagreement; or a weird coherent prediction about what ambitious people do next. Do not design a feature or implementation, manufacture prestige framing, or default to a competition slogan. Avoid permission, authority, workflow, release-gate, task-continuity, and benchmark-comparison takes.',
    ...
    hiddenConstraint: 'the sharp thought is usually about value, ambition, timing, a concrete company decision, or changed human behavior, not an invented product-management requirement',
    nonConsensusImplication: 'make the audacious named call or real question Geoffrey would say aloud without writing an AI industry recap or product specification',
    startupBackingFact: '',
  ...
  },
  {
    ...
    kind: 'ai_product',
    reactionPrompt: 'Keep the exact named company, person, or product. Make one audacious valuation or timing call with a company-specific reason, state one company-quality judgment, or ask one weird real question. Do not manufacture prestige framing or a generic competition slogan. Do not design permissions, workflows, governance, release gates, or benchmark tests. Do not default to an acquisition or CEO-installation fantasy and do not invent a launch.',
    ...
    hiddenConstraint: 'distribution matters, but the native reaction should expose the named company\'s value, ambition, timing, or concrete strategic choice rather than prescribe a generic workflow',
    nonConsensusImplication: 'make a specific company-value or competitive judgment instead of recapping a model launch or drafting a product roadmap',
    startupBackingFact: '',
...
function preferredGeoffreySeedKinds(targetTopic: string): Array<NonNullable<FrontierIdeaSeed['kind']>> {
  ...
  if (/\b(?:ai|model|openai|anthropic|claude|codex|software|developer|engineering|browser|database|cloud|open source|research|math|science)\b/.test(target)) return ['ai_product'];
