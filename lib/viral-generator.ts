...
Each post must contain a specific startup/company/market judgment, but it does not have to lead with the affected actor. A casual reaction to the subject can lead when the company, product, customer, market, price, cost, margin, capital, talent, supplier, or timing consequence becomes clear by the end. Silently decide what Geoffrey actually thinks and feels before drafting. The first sentence should sound like a thought, not an analyst thesis. The "one supplied fact" is optional backing, not an outline and not a request for an explainer. Do not summarize the source. If a brief does not support a sharp judgment without invention, write the narrowest defensible opinion and stop.
...
Make a human attitude legible without inventing experience. Surprise, excitement, skepticism, annoyance, amusement, and conviction are all valid; neutral mechanism summaries are not. Do not force every draft into who wins, loses, pays, or cannot ship.
