...
const PATTERN_RULES: PatternRule[] = [
  ...
  { label: 'synthetic-bar-open', weight: 0.52, pattern: /^(?:my|the)\s+bar\s+(?:for|on)\b[^:\n]{0,140}:/im },
  ...
  { label: 'not-merely-reframe', weight: 0.38, pattern: /,\s*not\s+merely\b/i },
  ...
  { label: 'named-subject-wins-when', weight: 0.42, pattern: /^(?:@?[a-z0-9][a-z0-9 .&'’/-]{0,60})\s+wins?\s+when\b/im },
  { label: 'one-layer-too-early', weight: 0.52, pattern: /\b(?:stops?|stopped|quit|quits|ends?|ended)\s+one\s+layer\s+too\s+early\b/i },
  { label: 'graded-reframe', weight: 0.52, pattern: /\bno longer\s+gets?\s+graded\s+on\b[\s\S]{0,180}\b(?:now\s+)?gets?\s+graded\s+on\b/i },
  { label: 'test-stops-becomes', weight: 0.42, pattern: /\b(?:bar|test|grade)\s+stops?\s+being\b[^.!?\n]{2,120}\b(?:and\s+)?becomes?\b/i },
  { label: 'no-room-left-merely', weight: 0.48, pattern: /\bhas no room left to be merely\b/i },
  { label: 'bar-is-now', weight: 0.38, pattern: /\bthe (?:bar|test|grade) is now\b/i },
  { label: 'whole-value-is', weight: 0.34, pattern: /\bthe whole value is\b/i },
  { label: 'moment-is-moment', weight: 0.42, pattern: /\bthe moment\b[^.!?\n]{3,140}\bis the moment\b/i },
  { label: 'that-is-company-i-want', weight: 0.42, pattern: /\bthat(?:['’]s| is) the company i want\b/i },
  { label: 'version-not-company', weight: 0.42, pattern: /\bthe [a-z][^.!?\n]{0,70} version is not the company\b/i },
  { label: 'needs-become-not-just', weight: 0.42, pattern: /\bneeds? to become\b[^.!?\n]{2,120},\s*not just\b/i },
  ...
  { label: 'simultaneous-balanced-reframe', weight: 0.52, pattern: /\bcan be\b[^.!?\n]{2,100}\band\b[^.!?\n]{2,100}\bat the same time[.!?]\s*(?:that|this)(?:['’]s| is) exactly when\b/i },
  ...
  { label: 'forecast-checklist-scaffold', weight: 0.52, pattern: /^[^.!?\n]{0,80}\bwill\b[^.!?\n]{2,140}\bwithin\s+(?:the\s+next\s+)?(?:\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\s+months?\b[^.!?\n]{0,120}\bas\s+(?:each\s+(?:[a-z0-9-]+\s+){0,2}(?:generation|model|cycle)|(?:reliability|capability|adoption|performance)\s+compounds?(?:\s+each\s+(?:[a-z0-9-]+\s+){0,2}(?:generation|model|cycle))?)\b/i },
  {
    label: 'horizon-mechanism-consequence-checklist',
    weight: 0.58,
    pattern: /^(?:within|in)\s+(?:the\s+next\s+)?(?:\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\s+months?\b(?=[\s\S]{0,220}\b(?:will|could|can|going\s+to|expect(?:ed)?\s+to)\b)(?=[\s\S]{0,220}\b(?:reliability|capability|adoption|performance|cost|latency|throughput|deployment|fleet\s+data|usage\s+data)\b)[\s\S]{12,260}(?:\n\s*\n|[.!?]\s+)(?:then\s+|so\s+)?(?:@?[a-z0-9_]+\s+becomes?\b|the\s+(?:person|company|startup|team|job|role|skill)\b[^.!?\n]{0,80}\b(?:needs?|becomes?|is)\b)/i,
  },
