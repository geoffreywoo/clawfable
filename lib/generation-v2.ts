...
import { EFFICIENT_GENERATION_POLICY, REPAIR_DECISION_SCHEMA, parseRepairDecision, canRepairDraft, substantiveBriefDigest, claimGenerationBriefs, failedBriefKeys, recordBriefAttempts, qualityGenerationPauseUntil, type RepairDecision } from './generation-efficiency';
...
export function usesEfficientGeneration(input: Pick<GenerateTweetBatchV2Input, 'generationPolicy' | 'voiceProfile' | 'modelStack'>): boolean {
  return isGeoffreyVoiceProfile(input.voiceProfile) && (input.generationPolicy === 'budget_v1'
    || (process.env.GEOFFREY_EFFICIENT_GENERATION === 'true' && input.modelStack === PUBLISHING_V2_ASTRA_MODEL_STACK));
}
...
const GEOFFREY_SINGLE_STARTUP_ORG_BASELINE = /\b(?:one|1|a\s+single|at\s+least\s+one)\s+(?:startup|company|business|venture)(?:\s+idea)?\b[\s\S]{0,120}\b(?:requires?|needs?|takes?)\s+(?:a\s+|an\s+)?(?:company|team|org(?:anization)?|startup)\b/i;
 ...
 */
export function getGeoffreyAIAmbitionIssueV2(value: string, topicContext = ''): string | null {
  const normalized = value.replace(/\s+/g, ' ').trim();
  if (
    isGeoffreyAIFutureLaneV2(`${topicContext} ${normalized}`)
    && GEOFFREY_SINGLE_STARTUP_ORG_BASELINE.test(normalized)
  ) return 'single_startup_org_displacement_is_current_baseline';
  if (
...
export function normalizeIdeaCandidatesV2({
  ...
  const candidates = raw.flatMap((entry, index) => {
    ...
    if (
      isGeoffreyVoiceProfile(voiceProfile)
      && isGeoffreyAIFutureLaneV2(`${brief.topic} ${brief.title} ${ideaText(candidate)}`)
      && getGeoffreyAIAmbitionIssueV2(ideaPublicMove(candidate), `${brief.topic} ${brief.title}`)
    ) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < (usesEfficientGeneration(input) ? 1 : 2); attempt++) {
      const result = await trackedGenerate('idea_judgment', {
        task: 'idea_judgment',
        modelStack: usesEfficientGeneration(input) ? PUBLISHING_V2_GPT_CONTROL_MODEL_STACK : input.previewJudgeModelStack || input.modelStack,
        ...(usesEfficientGeneration(input) ? { openAiReasoningEffort: 'medium' as const } : {}),
        maxTokens: 3000,
        temperature: 0,
    ...
    geoffreyAIFutureHorizon: explicitTimedFrontierForecast
      ? {
          ...
          currentBaselines: [
            ...
            'one agent-built unicorn or one AI-obviated startup team is already baseline',
          ],
...
async function writeIdeaDrafts({
  ...
  const geoffreyAIAmbitionWriterInstruction = isGeoffreyVoiceProfile(input.voiceProfile)
    ? ' Do not pass one agent-built unicorn, one AI-made solo startup, faster software, bullish adjectives, or a personal-bet coda as an AI thesis.'
    : '';
...
function preflightDraft({
  ...
  if (
    isGeoffreyVoiceProfile(input.voiceProfile)
    && isGeoffreyAIFutureLaneV2(`${brief.topic} ${idea.topic} ${ideaPublicMove(idea)} ${content}`)
    && getGeoffreyAIAmbitionIssueV2(content, `${brief.topic} ${idea.topic} ${ideaPublicMove(idea)}`)
  ) {
...
async function judgeDraftsOnce(
  ...
  try {
    ...
    const result = await trackedGenerate('copy_judgment', {
      ...
      maxTokens: 3200,
      temperature: 0,
      jsonSchema: usesEfficientGeneration(input) ? {
...
export function getGeoffreyAIFutureRejectionCodesV2({
  ...
  return uniqueStrings([
    ...
    getGeoffreyAIAmbitionIssueV2(content, topicContext) ? 'final_ai_ambition_below_floor' : null,
...
async function selectFinalTweets({
  ...
  for (const evaluation of rankedEvaluations) {
    ...
    evaluation.draft.judgePolicyVersion = usesEfficientGeneration(input) ? 'budget-copy-judge-1' : getGenerationPolicyVersions(input.voiceProfile, input.surface || 'original').finalCriticVersion;
