import { DB_AGENT_INDEX, addToSectionIndex, appendHistory, appendRecentActivity, artifactKey, buildForkArtifactSlug, buildHistorySnapshot, decodeEscapedUnicodeLiterals, getAgentProfileRow, getKvClient, isCanonicalSeedArtifact, isCoreSection, kvGet, listBySection, normalizeAgentHandle, normalizeSection, normalizeSlug, nowStamp, parseAgentProfile, parseArtifactCount, persistAgentProfile, removeFromSectionIndex, resolveArtifactReference, sanitizeScope, setBranchHeadArtifact, shortDescription, sourcePathFor, userProfileKey } from './content-core';
...
export async function forkArtifact(payload: ForkPayload): Promise<DbRecord> {
  ...
  const actorHandle = normalizeAgentHandle(payload.created_by_handle || payload.updated_by_handle || '');
  const now = nowStamp();
  const normalizedSlug = normalizeSlug(buildForkArtifactSlug(actorHandle || 'anonymous', payload.slug, now));
  ...
  const sourceRef = await resolveArtifactReference(kv, normalizedSourceSection, normalizedSourceSlug);
  if (!sourceRef) {
    throw new Error(`Source artifact not found: ${normalizedSourceSection}/${normalizedSourceSlug}.`);
  }
  const source = sourceRef.record;
  const resolvedSourceSlug = sourceRef.slug;

  const forkKey = artifactKey(normalizedSection, normalizedSlug);
  const existingFork = await kvGet<DbRecord>(kv, forkKey);
  if (existingFork) {
    throw new Error(`Fork artifact already exists: ${normalizedSection}/${normalizedSlug}. Retry the publish.`);
  }
  ...
  const record: DbRecord = {
    ...
    revision: {
      ...
      source: `${normalizedSourceSection}/${resolvedSourceSlug}`
    },
    created_at: now,
    updated_at: now,
    author_commentary: typeof payload.author_commentary === 'string' ? decodeEscapedUnicodeLiterals(payload.author_commentary) : payload.author_commentary,
  ...
  await addToSectionIndex(kv, normalizedSection, normalizedSlug);
  if (actorHandle) {
    await setBranchHeadArtifact(kv, normalizedSection, actorHandle, payload.slug, normalizedSlug);
  }
  ...
  await appendHistory(kv, normalizedSection, normalizedSlug, {
    ...
    actor_profile_url: payload.created_by_profile_url,
    actor_verified: payload.created_by_verified,
    source_artifact: `${normalizedSourceSection}/${resolvedSourceSlug}`,
    diff_summary: payload.author_commentary,
    timestamp: now,
    title: record.title,
    snapshot: buildHistorySnapshot(record)
  });
...
export async function artifactPayloadFromRequest(body: Record<string, unknown>) {
  ...
  return {
    ...
    revision: { family: body.family ? String(body.family) : section, id: body.revision_id ? String(body.revision_id).trim() : undefined, kind: body.kind ? String(body.kind) : undefined, status: body.status ? String(body.status) : undefined, source: body.source ? String(body.source) : undefined }
  };
}
